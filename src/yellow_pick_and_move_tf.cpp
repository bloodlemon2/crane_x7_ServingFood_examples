...
private:
  ...
  void picking(tf2::Vector3 target_position)
  {
    ...
    control_arm(target_position.x() - 0.07, target_position.y(), target_position.z() + 0.10, -180, -30, 0);
    // 掴みに行く
    ...
    // 持ち上げる
    control_arm(target_position.x() - 0.08, target_position.y(), target_position.z() + 0.12, -180, -30, 0);

    // 移動する
    control_arm(0.2, -0.4, 0.2, -180, -30, 0);

    // 下ろす
    control_arm(0.2, -0.4, 0.11, -180, -30, 0);

    // ハンドを開く
    control_gripper(GRIPPER_OPEN);

    // 少しだけハンドを持ち上げる
    control_arm(0.1, - 0.4, 0.2, -180, -30, 0);
